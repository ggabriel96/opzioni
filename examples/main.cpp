...
int main(int argc, char const *argv[]) {
  ...
  program.add(pos("name").help("Your first name"));

  program.add(opt("double", "d").help("A double").otherwise(7.11));
  program.add(opt("last-name").help("Your last name"));
  program.add(opt("o").help("We also support options with only short names").otherwise("oh"s));
  program.add(
      opt("num", "n").help("Creates a list of numbers with each appearence of this argument").action(append<int>));
  program.add(opt("str").help("Appends to a list of strings").action(append<std::string>).otherwise(std::vector{""s}));
  program.add(
      opt("vec")
          .otherwise(std::vector<int>{})
          .help(
              "This option uses the default action, so it'll not append to a list from each appearence in the CLI. It "
              "will instead parse a comma-separated list once"));
  program.add(opt("verbose", "v").help("Level of verbosity").set(1).otherwise(0));
  ...
  program.add(
      flg("append", "a")
          ...
          .help(
              "The equivalent of Python's argparse `append_const`: will append the defined value every time it appears "
              "in the CLI")
          .action(append<int>));
  program.add(flg("flag", "f")
                  .set("do something!"s)
                  .otherwise("nope"s)
                  .help("The equivalent of Python's argparse `store_const`: will store the defined value if it appears "
                        "in the CLI"));
  program.add(flg("t").help("We also support flags with only short names").otherwise(false));
