...
int main(int argc, char const *argv[]) {
  ...
  auto program =
      ...
      opt("o").help("We also support options with only short names").otherwise("oh"s) +
      opt("num", "n").help("Creates a list of numbers with each appearence of this argument").action(append<int>) +
      opt("str").help("Appends to a list of strings").action(append<std::string>).otherwise(std::vector{""s}) +
      opt("vec")
          .otherwise(std::vector<int>{})
          .help("This option uses the default action, so it'll not append to a list from each appearence in the "
                "CLI. It "
      ...
      flg("append", "a")
          ...
          .help("The equivalent of Python's argparse `append_const`: will append the defined value every time it "
                "appears "
                "in the CLI")
          .action(append<int>) +
      flg("flag", "f")
